...
chaincodeInvoke 1 0 "${ORG1_PEER0_URL}" "${ORG1_PEER0_TLS_ROOTCERT}" ${APP_CHANNEL} "${ORDERER0_URL}" "${ORDERER0_TLS_ROOTCERT}" ${CC_MARBLES_NAME} ${CC_MARBLES_INVOKE_INIT_ARGS}
...
for org in "${ORGS[@]}"; do
  for peer in "${PEERS[@]}"; do
    t="\${ORG${org}_PEER${peer}_URL}" && peer_url=$(eval echo $t)
    t="\${ORG${org}_PEER${peer}_TLS_ROOTCERT}" && peer_tls_rootcert=$(eval echo $t)
    chaincodeInstall $org $peer ${peer_url} ${peer_tls_rootcert} ${CC_NAME} ${CC_UPGRADE_VERSION} ${CC_PATH}
  done
done
...
chaincodeInvoke 1 0 "${ORG1_PEER0_URL}" "${ORG1_PEER0_TLS_ROOTCERT}" ${APP_CHANNEL} "${ORDERER0_URL}" "${ORDERER0_TLS_ROOTCERT}" ${CC_MARBLES_NAME} ${CC_MARBLES_INVOKE_INIT_ARGS_2}
echo_g "=== Invoke chaincode done ==="
