...
# Resolve an installed package id for the given chaincode label.
# Prefer the deterministic package file mounted into /tmp/scripts when present,
# and fall back to queryinstalled only when there is a single matching label.
resolveInstalledPackageID() {
  if [ "$#" -lt 1 -o "$#" -gt 2 ]; then
    echo_r "Wrong param number for resolving package id"
    exit -1
  fi

  local label=$1
  local package_file=${2:-/tmp/scripts/${label}.tar.gz}
  local matches
  local match_count

  if [ -f "${package_file}" ]; then
    peer lifecycle chaincode calculatepackageid "${package_file}" >"$RUN_LOG_FILE" 2>&1
    rc=$?
    [ $rc -ne 0 ] && cat "$RUN_LOG_FILE"
    verifyResult $rc "Failed to calculate package id from ${package_file}"
    tail -n 1 "$RUN_LOG_FILE"
    return 0
  fi
  ...
  verifyResult $rc "Failed to query installed chaincode packages"

  matches=$(grep -o "${label}:[a-z0-9]*" "$QUERY_LOG_FILE" || true)
  ...
  if [ "${match_count}" = "0" ]; then
    echo_r "Cannot find installed package id for label ${label}"
  else
    echo_r "Found multiple installed package ids for label ${label}; clean stale packages or provide the package file explicitly"
  fi
  exit -1
}
...
chaincodeGetInstalled() {
  ...
  package_id=$(resolveInstalledPackageID "${cc_name}")
...
chaincodeInstantiate() {
  echo_r "Legacy 'peer chaincode instantiate' is not supported in the Fabric 3.1 example."
...
chaincodeUpgrade() {
  echo_r "Legacy 'peer chaincode upgrade' is not supported in the Fabric 3.1 example."
