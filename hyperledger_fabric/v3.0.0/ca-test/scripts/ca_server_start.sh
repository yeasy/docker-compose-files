...
if [ ! -d ${CA_PATH} ]; then
	...
	yq eval -i '.ca.name = strenv(TYPE) + "." + strenv(ORG_NAME)' fabric-ca-server-config.yaml
	yq eval -i '.ca.certfile = strenv(TYPE) + "." + strenv(ORG_NAME) + "-cert.pem"' fabric-ca-server-config.yaml
	yq eval -i '.ca.keyfile = strenv(TYPE) + "." + strenv(ORG_NAME) + "_sk"' fabric-ca-server-config.yaml

	yq eval -i '.csr.cn = strenv(TYPE) + "." + strenv(ORG_NAME)' fabric-ca-server-config.yaml
	yq eval -i '.csr.names[0].O = strenv(ORG_NAME)' fabric-ca-server-config.yaml
	yq eval -i '.csr.names[0].OU = strenv(TYPE)' fabric-ca-server-config.yaml

	yq eval -i '.tls.enabled = false' fabric-ca-server-config.yaml
